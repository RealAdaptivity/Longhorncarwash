...
async function sendAiMessage(message) {
  ...
  userMsg.style.cssText = 'background:var(--primary);padding:10px;border-radius:10px;color:white;align-self:flex-end;max-width:85%;line-height:1.4;';
  if (state.aiSelectedImageBase64) {
    userMsg.innerHTML = `<img src="data:${state.aiSelectedImageMime};base64,${state.aiSelectedImageBase64}" style="max-width:100%;border-radius:5px;margin-bottom:5px;" /><br>`;
  ...
  typingMsg.style.cssText = 'background:rgba(142,68,173,0.1);border:1px solid rgba(142,68,173,0.3);padding:10px;border-radius:10px;color:var(--text-muted);align-self:flex-start;max-width:85%;line-height:1.4;';
  ...
  if (!apiKey) {
    typingMsg.textContent = 'Error: Gemini API key not configured. Set it in Settings.';
  ...
  if (postScheduleSection && !postScheduleSection.classList.contains('hidden')) {
    ...
    const employees = Array.from(document.querySelectorAll('#schedule-editor-body tr')).map(tr => tr.querySelector('td strong')?.innerText || '');
    context = `CURRENT UI CONTEXT: The manager is viewing the Schedule Editor. Employees: ${employees.join(', ')}. Days: ${headers.join(', ')}. If asked to generate a schedule, return a JSON block in \`\`\`json format like [{"employee": "Name", "shifts": ["8:00-4:00", "-", "9:00-5:00", ...]}]. Give 30-40 hours total, 2 days off ("-").`;
  }

  const parts = [{ text: `You are a helpful AI assistant for Longhorn Car Wash.\n${context}\n\nUser says: ${message}` }];
  if (state.aiSelectedImageBase64) {
    parts.push({ inline_data: { mime_type: state.aiSelectedImageMime, data: state.aiSelectedImageBase64 } });
  }

  // Clear attachment before sending
  const btnRemoveAiImage = document.getElementById('btn-remove-ai-image');
  if (btnRemoveAiImage) btnRemoveAiImage.click();
      ...
      try {
        ...
        document.querySelectorAll('#schedule-editor-body tr').forEach(tr => {
          ...
          if (empData && empData.shifts) {
            tr.querySelectorAll('.sched-cell').forEach((inp, idx) => { if (empData.shifts[idx]) inp.value = empData.shifts[idx]; });
          }
        });
...
export function init() {
  ...
  if (aiFab) {
    aiFab.addEventListener('click', () => { if (aiChatPanel) aiChatPanel.classList.toggle('hidden'); });
    if (btnCloseAi) btnCloseAi.addEventListener('click', () => { if (aiChatPanel) aiChatPanel.classList.add('hidden'); });

    if (btnUploadAi) btnUploadAi.addEventListener('click', () => { if (aiChatFile) aiChatFile.click(); });
    ...
    if (aiChatFile) {
      aiChatFile.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => {
          ...
          state.aiSelectedImageMime = meta.match(/:(.*?);/)[1];
          state.aiSelectedImageBase64 = base64;
          if (aiImagePreviewName) aiImagePreviewName.textContent = file.name;
          if (aiImagePreviewContainer) aiImagePreviewContainer.classList.remove('hidden');
        };
    ...
    if (btnRemoveAiImage) {
      btnRemoveAiImage.addEventListener('click', () => {
        state.aiSelectedImageBase64 = null;
        state.aiSelectedImageMime = null;
        if (aiChatFile) aiChatFile.value = '';
        if (aiImagePreviewContainer) aiImagePreviewContainer.classList.add('hidden');
      });
    }

    const sendMsg = () => {
      const msg = aiChatInput ? aiChatInput.value.trim() : '';
      if (msg || state.aiSelectedImageBase64) {
        if (aiChatInput) aiChatInput.value = '';
        sendAiMessage(msg || 'Look at this image.');
      }
    };

    if (btnSendAi) btnSendAi.addEventListener('click', sendMsg);
    if (aiChatInput) aiChatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') sendMsg(); });
  }
}
